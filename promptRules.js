...
function buildMinimumLengthRule(minChars, scene = 'chat') {
  ...
  if (scene === 'chat') {
    return `${noun}当前设置的最低长度约为 ${minimum} 个中文字符。该数字是柔性下限，但正式 Chat 默认应把它当作正常展开的参考线：除非叶檀明确要求简短、只问一个很简单的事实问题，或当前内容确实只需要一句话，否则不要过早结束。通常围绕用户当前话题自然展开 2—5 个有内容的段落，优先补充回应、情绪承接、具体细节和自然延伸，让对话有来有回；可以少于下限，但不要长期稳定地明显短于它。严禁为了达到长度机械凑字、重复同一句意思、堆砌形容词或无关发散。`;
  }
