...
// the Theater UI style or inventing a second paid "thinking" completion.
...
function isTheaterAssistantInsert(body) {
  ...
  return body.category === '小剧场互动' && body.author === '泽' && typeof body.content === 'string';
}
    ...
    try {
      ...
      if (isTheaterProviderBody(prepared)) {
        response.clone().json().then(payload => {
          ...
          collect(payload);
          const answer = texts.map(String).find(text => text.trim() && text.trim() !== thinking.trim()) || '';
          if (thinking && answer) remember(answer, thinking);
        }).catch(() => {});
      }
      return response;
    } catch (error) {
...
// Express response enrichment makes native thinking available to the existing
// Theater frontend without changing its visual shell. It also picks up stored
// reasoning when the letters REST response was fetched during this process.
try {
...
module.exports = { THEATER_RE, modelSupportsNativeThinking, maybeEnableThinking, remember, lookup, enrichResponseBody };
