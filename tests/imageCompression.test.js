...
const { compressImageBuffer, isCompressibleImageType, normalizeImageType } = require('../imageCompression');
...
test('shrinks a large photo while preserving its media type when Sharp is available', { skip: !sharp }, async () => {
  const width = 3200, height = 2400;
  const pixels = Buffer.allocUnsafe(width * height * 3);
  for (let index = 0; index < pixels.length; index += 3) {
    const pixel = index / 3, x = pixel % width, y = Math.floor(pixel / width);
    pixels[index] = (x * 17 + y * 11) % 256;
    pixels[index + 1] = (x * 7 + y * 19) % 256;
    pixels[index + 2] = (x * 13 + y * 5) % 256;
  }
  const raw = await sharp(pixels, { raw: { width, height, channels: 3 } }).jpeg({ quality: 100 }).toBuffer();
...
test('leaves small images byte-for-byte unchanged without needing Sharp', async () => {
  const raw = Buffer.from('tiny-image-fixture');
...
test('large compressible images degrade safely when Sharp is unavailable', async () => {
  if (sharp) return;
  const raw = Buffer.alloc(800_000, 7);
  const result = await compressImageBuffer(raw, 'image/jpeg', { minBytes: 1 });
  assert.equal(result.compressed, false);
