...
function enabledConfig(mode = 'primary') {
  return {
    selectedProvider: 'oss', requestedMode: mode, mode, configured: true, enabled: true,
    primary: mode === 'primary', mirrorToSupabase: false, region: 'oss-cn-hangzhou',
    accessKeyId: 'id-secret', accessKeySecret: 'key-secret', bucket: 'ourhome-private', endpoint: '',
  };
}
...
test('Supabase remains primary even when stale OSS mode and credentials survive', () => {
  ...
  const env = { OURHOME_OSS_STORAGE_MODE: 'primary', ALIYUN_OSS_REGION: 'oss-cn-hangzhou', ALIYUN_OSS_ACCESS_KEY_ID: 'id', ALIYUN_OSS_ACCESS_KEY_SECRET: 'secret', ALIYUN_OSS_BUCKET: 'bucket' };
...
test('OSS read-only probe reports readiness without writing an object', async () => {
  let lists = 0;
  const env = { OURHOME_OBJECT_STORAGE_PRIMARY: 'oss', OURHOME_OSS_STORAGE_MODE: 'shadow', ALIYUN_OSS_REGION: 'oss-cn-beijing', ALIYUN_OSS_ACCESS_KEY_ID: 'id', ALIYUN_OSS_ACCESS_KEY_SECRET: 'secret', ALIYUN_OSS_BUCKET: 'bucket' };
  const result = await probeOssStorage({ env, storage: { async listObjects(options) { lists += 1; assert.deepEqual(options, { limit: 1 }); return []; } } });
  assert.equal(result.ok, true); assert.equal(lists, 1); assert.equal(ossStorageHealthStatus(env), 'shadow-ready');
});
...
test('OSS storage preserves paths, private caching and verification metadata', async () => {
  ...
  const client = {
    ...
    async head() { return { res: { headers: { 'content-length': '5', [HASH_HEADER]: 'abc', 'content-type': 'image/jpeg' } } }; },
  ...
  const uploaded = await oss.putObject('chat/饭.jpg', new Blob(['photo']), { contentType: 'image/jpeg', metadata: { [HASH_HEADER]: 'abc' } });
...
test('signed image reads stay on Supabase until each OSS object exists', async () => {
  ...
  const bridge = createSignedUrlBridge({
    oss: { async headObject(path) { return path === 'ready.jpg' ? { size: 5 } : null; }, signedUrl(path) { return `https://oss.example/${path}`; } },
    async originalSignOne(path) { sourceCalls.push(path); return { data: { path, signedUrl: `https://supabase.example/${path}` }, error: null }; },
    async originalSignMany(paths) { sourceCalls.push(...paths); return { data: paths.map(path => ({ path, signedUrl: `https://supabase.example/${path}`, error: null })), error: null }; },
  });
  ...
  assert.deepEqual(batch.data.map(item => item.signedUrl), ['https://oss.example/ready.jpg', 'https://supabase.example/waiting.jpg']);
...
test('OSS probe errors fall back to the Supabase image instead of returning a broken link', async () => {
  const bridge = createSignedUrlBridge({
    oss: { async headObject() { throw new Error('temporary OSS probe failure'); }, signedUrl() { throw new Error('must not sign an unverified target'); } },
    async originalSignOne(path) { return { data: { path, signedUrl: `https://supabase.example/${path}` }, error: null }; },
    async originalSignMany(paths) { return { data: paths.map(path => ({ path, signedUrl: `https://supabase.example/${path}` })), error: null }; },
  });
...
test('migration CLI is dry-run by default and apply must be explicit', () => {
  assert.deepEqual(parseArgs([]), { apply: false, includeExpired: false, bucket: 'uploads', concurrency: 2, limit: Infinity, batch: Infinity, retentionDays: 30 });
...
test('Supabase inventory walks folders and paginates without inventing paths', async () => {
  const calls = [];
  const fileApi = { async list(prefix) { calls.push(prefix); if (!prefix) return { data: [{ id: null, name: 'chat', metadata: null }, { id: '1', name: 'root.jpg', metadata: { size: 4 } }], error: null }; return { data: [{ id: '2', name: '饭.jpg', metadata: { size: 5, mimetype: 'image/jpeg' } }], error: null }; } };
  const result = await listSupabaseObjects(fileApi);
  assert.deepEqual(calls, ['', 'chat']); assert.deepEqual(result.map(item => item.path), ['root.jpg', 'chat/饭.jpg']);
});

test('migration verifies hash and skips an already-copied target', async () => {
  let writes = 0; const bytes = Buffer.from('photo'); const digest = sha256(bytes);
  const fileApi = { async download() { throw new Error('download should be avoided only after source hash is known'); } };
  const oss = { async headObject() { return { size: bytes.length, headers: { [HASH_HEADER]: digest } }; } };
  fileApi.download = async () => ({ data: new Blob([bytes]), error: null });
  const result = await migrateOne({ fileApi, oss: { ...oss, async putObject() { writes += 1; } }, source: { path: 'a.jpg', size: bytes.length, contentType: 'image/jpeg' }, apply: true });
  assert.equal(result.action, 'verified'); assert.equal(writes, 0);
});

test('migration resumes without downloading an object stamped from the same source version', async () => {
  const bytes = Buffer.from('photo'); const updatedAt = '2026-08-12T03:04:05.000Z';
  const fileApi = { async download() { throw new Error('verified marker should avoid a second download'); } };
  const result = await migrateOne({ fileApi, oss: { async headObject() { return { size: bytes.length, headers: { [HASH_HEADER]: sha256(bytes), [SOURCE_UPDATED_HEADER]: updatedAt } }; } }, source: { path: 'a.jpg', size: bytes.length, contentType: 'image/jpeg', updatedAt }, apply: true });
...
test('retained migration skips only expired ordinary images with saved analyses', async () => {
  ...
  const rows = [{ id: 1, created_at: '2026-06-01T00:00:00.000Z', attachment_url: `${base}expired.jpg`, attachment_type: 'image/jpeg', attachment_name: 'expired.jpg', attachment_summary: '已经确认的画面描述' }];
  const supabase = { from(table) { if (table === 'messages') return { select() { return { async not() { return { data: rows, error: null }; } }; } }; return { async select() { return { data: [], error: null }; } }; } };
  const inventory = [{ path: 'expired.jpg', size: 10 }, { path: 'album.jpg', size: 20 }];
  const selection = await selectRetainedObjects(supabase, inventory, { retentionDays: 30, now: new Date('2026-08-12T00:00:00.000Z') });
  assert.deepEqual(selection.selected.map(item => item.path), ['album.jpg']); assert.deepEqual(selection.expired.map(item => item.path), ['expired.jpg']);
});
