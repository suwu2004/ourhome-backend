...
// keeping the original timestamps available as explicit temporal metadata.
...
function splitHistoryEntries(text) {
  ...
  return raw.split(/\n\n(?=(?:\d+\.\s*)?(?:【[^】]+】)?[^\n]{1,80}：)/u).map(item => item.trim()).filter(Boolean).map(item => {
...
function buildTimelineInstruction() {
  return `${TIME_MARKER}\n现在是 ${currentShanghaiTime()}（北京时间，Asia/Shanghai）。这是本轮请求的真实现实时间锚点。\n\n${JUMP_MARKER}\n默认：严格承接最近一次正在发生的剧情，不把旧消息误当成现在。\n允许：用户明确说“第二天、几天后、半年后、十年后、多年以后、后来、回到三年前、回忆起、与此同时、镜头转到”等时间跳跃/倒叙/平行场景时，立即建立新的剧情时间点并继续演绎。\n规则：时间跳跃后的后续剧情默认沿用新的时间点；明确回忆/倒叙属于临时过去场景，不自动覆盖主时间线；“与此同时”可建立并行场景，不把两地事件强行合并成同一时刻。\n如果用户只使用模糊的“过了一会儿/后来”而没有明确跨度，只做自然、短距离推进，不擅自跨越数月或数年。\n如果旧的整理记忆与最近真实对话冲突，以最近真实对话和本轮明确时间跳跃为准。`;
}
...
function buildStructuredMessages(body) {
  ...
  const currentUserName = currentMatch[1].trim(), currentText = currentMatch[2].trim();
  ...
  for (const entry of entries) {
    ...
    else historyMessages.push({ role, content: entry.text });
  }
