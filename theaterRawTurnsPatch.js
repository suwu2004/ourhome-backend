...
function splitHistoryEntries(text) {
  ...
  if (/^\s*\d+\.\s*/u.test(raw)) return raw.split(/\n(?=\s*\d+\.\s*)/u).map(parseHistoryChunk).filter(Boolean);
  return raw.split(/\n\s*(?=(?:【[^】]+】)?[^：\n]{1,80}：)/u).map(parseHistoryChunk).filter(Boolean);
}
...
function normalizeTurnText(value) { return String(value || '').replace(/^【当前剧情时间待判定】\s*/u, '').replace(/^【历史剧情时间：[^】]+（Asia\/Shanghai）】\s*/u, '').replace(/\s+/gu, ' ').trim(); }
...
function buildStructuredMessages(body) {
  ...
  let recentMessages = historyMessages.length > RECENT_MESSAGE_WINDOW ? historyMessages.slice(-RECENT_MESSAGE_WINDOW) : historyMessages;
  if (recentMessages.length && recentMessages[0].role === 'assistant') recentMessages = recentMessages.slice(1);
...
if (typeof previousFetch === 'function') {
  globalThis.fetch = async function theaterRawTurnsFetch(input, init = {}) {
    ...
    } catch (error) { console.warn('[theater:raw-turns] skipped:', error.message); return previousFetch(input, init); }
  };
}
